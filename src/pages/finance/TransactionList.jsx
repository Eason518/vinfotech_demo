...
export default function TransactionList() {
  const [filter, setFilter] = useState({ txnType: '', status: '', from: '', to: '', search: '', description: '' });
  ...
  const filtered = transactions.filter(t =>
    ...
    (!filter.search || t.username.toLowerCase().includes(filter.search.toLowerCase()) ||
      ...
      t.userId.toLowerCase().includes(filter.search.toLowerCase()))
  );
  ...
  return (
    <Layout title="Transaction">
      ...
      <div className="card">
        ...
        <div className="filter-row" style={{ flexWrap: 'wrap', marginBottom: '16px' }}>
          <div className="form-group">
            <label className="form-label">Transaction Type</label>
            <select className="form-control" value={filter.txnType} onChange={e => setFilter({ ...filter, txnType: e.target.value })}>
              <option value="">All</option>
              <option>Deposit</option><option>Withdrawal</option><option>Winning</option><option>Refund</option>
            </select>
          </div>
          <div className="form-group">
          ...
          <div className="form-group">
            <label className="form-label">User ID / Username / Email / Phone</label>
            <input className="form-control" style={{ minWidth: '220px' }} placeholder="Search..." value={filter.search} onChange={e => setFilter({ ...filter, search: e.target.value })} />
          </div>
          <div className="form-group">
            <label className="form-label">Description</label>
            <select className="form-control" value={filter.description} onChange={e => setFilter({ ...filter, description: e.target.value })}>
              <option value="">All</option>
              <option>Admin</option><option>User</option><option>System</option>
              <option>Deposit</option><option>Contest Join</option><option>Winning</option><option>Refund</option>
            </select>
          </div>
          <button className="btn btn-outline btn-sm" style={{ alignSelf: 'flex-end' }} onClick={() => setFilter({ txnType: '', status: '', from: '', to: '', search: '', description: '' })}>
            Clear Filters
          </button>
        </div>

        <div style={{ fontWeight: 700, fontSize: '16px', marginBottom: '12px' }}>Transactions</div>
